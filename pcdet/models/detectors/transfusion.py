...
class TransFusion(Detector3DTemplate):
    ...
    def get_training_loss(self, batch_dict):
        ...
        # OpenPCDet's TransFusion head returns aggregated loss in batch_dict
        if 'loss' in batch_dict and 'tb_dict' in batch_dict:
            loss_trans = batch_dict['loss']
            tb_dict = {'loss_trans': loss_trans.item(), **batch_dict['tb_dict']}
            return loss_trans, tb_dict, disp_dict

        loss_rpn, tb_dict = self.dense_head.get_loss()
        tb_dict = {'loss_trans': loss_rpn.item(), **tb_dict}
